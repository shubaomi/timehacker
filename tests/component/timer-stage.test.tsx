...
describe("TimerStage", () => {
  it("exposes READY, RUNNING, and STOP controls", async () => {
    ...
    const { rerender } = render(<TimerStage {...baseProps} onPrimary={onPrimary} />);
    const start = screen.getByRole("button", { name: /START.*SPACE/i });
    await userEvent.click(start);
    expect(onPrimary).toHaveBeenCalledOnce();
    rerender(<TimerStage {...baseProps} status="RUNNING" elapsedMs={2_345} onPrimary={onPrimary} />);
  ...
  it("supports keyboard activation and reports keyboard input", async () => {
    const onPrimary = vi.fn();
    const onEvent = vi.fn();
    render(<TimerStage {...baseProps} onPrimary={onPrimary} onEvent={onEvent} />);
    const start = screen.getByRole("button", { name: /START.*SPACE/i });
    start.focus();
